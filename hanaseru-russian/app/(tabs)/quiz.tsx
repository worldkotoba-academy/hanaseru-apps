...
export default function QuizScreen() {
  ...
  const [direction, setDirection] = useState<TranslationDirection>("ru-ja");
  ...
  const startQuiz = useCallback(() => {
    ...
    const qs: QuizQuestion[] = selected.map((word) => {
      if (mode === "multiple") {
        ...
        const answerText =
          direction === "ru-ja" ? word.meaning : word.word;
        const choiceTexts = others.map((o) =>
          direction === "ru-ja" ? o.meaning : o.word
        );
      ...
      >
        <ScrollView contentContainerStyle={styles.setupContent}>
          ...
          <View style={styles.chipRow}>
            {(
              [
                { key: "ru-ja", label: "ロシア語→日本語" },
                { key: "ja-ru", label: "日本語→ロシア語" },
              ] as const
            ).map((d) => (
              <TouchableOpacity
                key={d.key}
  ...
  if (quizState === "playing" && questions.length > 0) {
    const q = questions[currentIndex];
    const questionText =
      direction === "ru-ja" ? q.word.word : q.word.meaning;
    const answerText =
      direction === "ru-ja" ? q.word.meaning : q.word.word;
      ...
      >
        ...
        <View style={styles.playContent}>
          ...
          >
            ...
            {direction === "ru-ja" && q.word.pronunciation ? (
              <Text
