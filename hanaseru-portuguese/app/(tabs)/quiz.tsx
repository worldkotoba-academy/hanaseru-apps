...
export default function QuizScreen() {
  ...
  const [direction, setDirection] = useState<TranslationDirection>("pt-ja");
  ...
  const startQuiz = useCallback(() => {
    ...
    const qs: QuizQuestion[] = selected.map((word) => {
      if (mode === "multiple") {
        ...
        const answerText =
          direction === "pt-ja" ? word.meaning : word.word;
        const choiceTexts = others.map((o) =>
          direction === "pt-ja" ? o.meaning : o.word
        );
      ...
      >
        <ScrollView contentContainerStyle={styles.setupContent}>
          ...
          <View style={styles.chipRow}>
            {(
              [
                { key: "pt-ja", label: "ポルトガル語→日本語" },
                { key: "ja-pt", label: "日本語→ポルトガル語" },
              ] as const
            ).map((d) => (
              <TouchableOpacity
                key={d.key}
  ...
  if (quizState === "playing" && questions.length > 0) {
    const q = questions[currentIndex];
    const questionText =
      direction === "pt-ja" ? q.word.word : q.word.meaning;
    const answerText =
      direction === "pt-ja" ? q.word.meaning : q.word.word;
      ...
      >
        ...
        <View style={styles.playContent}>
          ...
          >
            ...
            {direction === "pt-ja" && q.word.pronunciation ? (
              <Text
