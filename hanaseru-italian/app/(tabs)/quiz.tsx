...
export default function QuizScreen() {
  ...
  const [direction, setDirection] = useState<TranslationDirection>("it-ja");
  ...
  const startQuiz = useCallback(() => {
    ...
    const qs: QuizQuestion[] = selected.map((word) => {
      if (mode === "multiple") {
        ...
        const answerText =
          direction === "it-ja" ? word.meaning : word.word;
        const choiceTexts = others.map((o) =>
          direction === "it-ja" ? o.meaning : o.word
        );
      ...
      >
        <ScrollView contentContainerStyle={styles.setupContent}>
          ...
          <View style={styles.chipRow}>
            {(
              [
                { key: "it-ja", label: "イタリア語→日本語" },
                { key: "ja-it", label: "日本語→イタリア語" },
              ] as const
            ).map((d) => (
              <TouchableOpacity
                key={d.key}
  ...
  if (quizState === "playing" && questions.length > 0) {
    const q = questions[currentIndex];
    const questionText =
      direction === "it-ja" ? q.word.word : q.word.meaning;
    const answerText =
      direction === "it-ja" ? q.word.meaning : q.word.word;
      ...
      >
        ...
        <View style={styles.playContent}>
          ...
          >
            ...
            {direction === "it-ja" && q.word.pronunciation ? (
              <Text
