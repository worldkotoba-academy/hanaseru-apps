...
export default function QuizScreen() {
  ...
  const [direction, setDirection] = useState<TranslationDirection>("de-ja");
  ...
  const startQuiz = useCallback(() => {
    ...
    const qs: QuizQuestion[] = selected.map((word) => {
      if (mode === "multiple") {
        ...
        const answerText =
          direction === "de-ja" ? word.meaning : word.word;
        const choiceTexts = others.map((o) =>
          direction === "de-ja" ? o.meaning : o.word
        );
      ...
      >
        <ScrollView contentContainerStyle={styles.setupContent}>
          ...
          <View style={styles.chipRow}>
            {(
              [
                { key: "de-ja", label: "ドイツ語→日本語" },
                { key: "ja-de", label: "日本語→ドイツ語" },
              ] as const
            ).map((d) => (
              <TouchableOpacity
                key={d.key}
  ...
  if (quizState === "playing" && questions.length > 0) {
    const q = questions[currentIndex];
    const questionText =
      direction === "de-ja" ? q.word.word : q.word.meaning;
    const answerText =
      direction === "de-ja" ? q.word.meaning : q.word.word;
      ...
      >
        ...
        <View style={styles.playContent}>
          ...
          >
            ...
            {direction === "de-ja" && q.word.pronunciation ? (
              <Text
